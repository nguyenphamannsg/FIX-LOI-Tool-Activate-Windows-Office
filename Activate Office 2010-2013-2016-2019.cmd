...
for /f "tokens=*" %%b in ('powershell -Command "$req = [System.Net.WebRequest]::Create('https://winoffice.org/public-api/get-cid?iid=%IID%&token=gAAAAABfGoQup9m1LXzXr9_TalnklPYNAMWoRq8EfBGKHQK9DocWCGvyzZNQuCqGn7F4r0K5R2vPS8OyLMixD88MS_RmfeZrkyXwhbA0_TyYfeHqC6ZJX3s=');$resp = New-Object System.IO.StreamReader $req.GetResponse().GetResponseStream(); $resp.ReadToEnd()"') do set ACID=%%b
set CID=%ACID:~1,48%
...
@echo 3. Dang Xuat Installion ID va Get CID kich hoat Office
for /f "tokens=8" %%b in ('cscript ospp.vbs /dinstid ^| findstr /b /c:"Installation ID"') do set IID=%%b
for /f "tokens=*" %%b in ('powershell -Command "$req = [System.Net.WebRequest]::Create('https://huyphung.com/public-api/get-cid?iid=%MyIID% & token=gAAAAABfGoQup9m1LXzXr9_TalnklPYNAMWoRq8EfBGKHQK9DocWCGvyzZNQuCqGn7F4r0K5R2vPS8OyLMixD88MS_RmfeZrkyXwhbA0_TyYfeHqC6ZJX3s=');$resp = New-Object System.IO.StreamReader $req.GetResponse().GetResponseStream(); $resp.ReadToEnd()"') do set ACID=%%b
set CID=%ACID:~1,48%
echo CID cua ban %CID% 
@echo 5. Dang kich hoat ban quyen
Cscript ospp.vbs /actcid:%CID% & cscript ospp.vbs /act 
